...
RESULTS_DIR = "results/image_sensor_v2_cv_noaug"
...
def build_augmentation():
    # AUGMENTATION DISABLED -- identity passthrough.
    # Control run: same 5-fold CV protocol as train_image_sensor_v2.py,
    # but with no augmentation, to isolate whether augmentation or the
    # switch to 5-fold CV itself explains the accuracy change.
    return tf.keras.Sequential([
        tf.keras.layers.Lambda(lambda x: x, name="no_augmentation"),
    ], name="augmentation")
...
def main():
    ...
    summary = {
        ...
        "n_folds": N_FOLDS,
        "augmentation": ["DISABLED -- control run for comparison against augmented CV"],
    }
    ...
    plt.title("5-Fold Cross-Validation: Image+Sensor (NO augmentation -- control)")
    plt.legend()
    plt.tight_layout()
